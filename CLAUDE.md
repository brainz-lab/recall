...
cd /Users/afmp/brainz/brainzlab/recall
...
kamal app exec -p --reuse -e SECRET_KEY_BASE:<master_key> 'bin/rails runner "puts LogEntry.count"'
```
...
LogEntry.where(level: "error").limit(10).each { |e| puts e.message }
RUBY
...
ssh <user>@<primary-server> 'CONTAINER=$(docker ps --filter "name=recall-web" --format "{{.Names}}" | head -1) && \
